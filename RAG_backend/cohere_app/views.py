...
from .models import PromptHistory, CurrentUsingCollection, UserNames
...
def cohere_generate(request):
    try:
        ...
        usernames_entry = UserNames.objects.get(users=user_name)
...
def get_prompt_history(request):
    ...
    usernames_entry = UserNames.objects.get(users=request.user.username)
...
def mark_satisfied_or_unsatisfied(request, pk):
    ...
    try:
        ...
        feedback_status = data.get('status')
        usernames_entry = UserNames.objects.get(users=request.user.username)
        prompt_history = PromptHistory.objects.get(pk=pk, user=usernames_entry)
